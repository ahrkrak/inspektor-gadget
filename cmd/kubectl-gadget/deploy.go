...
func init() {
	...
	deployCmd.PersistentFlags().MarkDeprecated("verify-gadgets", "This flag is deprecated and will be removed in a future release. Use --set-configmap=operator.oci.verify-image=<true|false> instead")
	...
	deployCmd.PersistentFlags().MarkDeprecated("gadgets-public-keys", "This flag is deprecated and will be removed in a future release. Use --configmap-override-path instead")
	...
	deployCmd.PersistentFlags().MarkDeprecated("allowed-gadgets", "This flag is deprecated and will be removed in a future release. Use --configmap-override-path instead")
	deployCmd.PersistentFlags().StringSliceVar(
		&insecureRegistries,
		"insecure-registries",
		[]string{},
	...
	deployCmd.PersistentFlags().MarkDeprecated("insecure-registries", "This flag is deprecated and will be removed in a future release. Use --configmap-override-path instead")
	deployCmd.PersistentFlags().BoolVar(
		&disallowGadgetsPull,
		"disallow-gadgets-pulling", false, "Disallow pulling gadgets from registries")
	deployCmd.PersistentFlags().MarkDeprecated("disallow-gadgets-pulling", "This flag is deprecated and will be removed in a future release. Use --set-configmap=operator.oci.disallow-pulling=<true|false> instead")
	deployCmd.PersistentFlags().BoolVar(
		&otelMetricsListen,
		"otel-metrics-listen", false, "Enable OpenTelemetry metrics listener (Prometheus compatible) endpoint")
	deployCmd.PersistentFlags().MarkDeprecated("otel-metrics-listen", "This flag is deprecated and will be removed in a future release. Use --set-configmap=operator.otel-metrics.otel-metrics-listen=<true|false> instead")
	deployCmd.PersistentFlags().StringVar(
	...
	deployCmd.PersistentFlags().MarkDeprecated("otel-metrics-listen-address", "This flag is deprecated and will be removed in a future release. Use --set-configmap=operator.otel-metrics.otel-metrics-listen-address=<address> instead")
	deployCmd.PersistentFlags().StringSliceVar(
		&setConfigmap,
		"set-configmap", []string{}, "Set a key-value pair in the ConfigMap. The format is key=value. This flag can be used multiple times to set multiple values. Only recommended for primitive values (string, int, bool). For complex values, use --configmap-override-path")
	deployCmd.PersistentFlags().StringVar(
		&configmapOverridePath,
		"configmap-override-path", "", "Path to a config file to override the values in the ConfigMap. The file should be in YAML format")
...
// applyConfigToConfgMap overrides the ConfigMap using the following order of precedence (highest to lowest):
// 1. Flags (--gadgets-public-keys, --disallow-gadgets-pulling, etc.) [Deprecated]
// 2. --set-configmap flag
// 3. ConfigFile
// 4. ConfigMap default values
// 5. Flags default values [Deprecated]
func applyConfigToConfgMap(cm *v1.ConfigMap, configPath string, flags *pflag.FlagSet) error {
	if cm == nil {
	...
	if configPath != "" {
		overrideCfg, err := os.ReadFile(configPath)
		if err != nil {
			return fmt.Errorf("reading override config file %q: %w", configPath, err)
		}
		err = v.MergeConfig(strings.NewReader(string(overrideCfg)))
		if err != nil {
			return fmt.Errorf("merging config file %q: %w", configPath, err)
		}
	}

	// Set values from --set-configmap flag
	for _, set := range setConfigmap {
		parts := strings.SplitN(set, "=", 2)
		if len(parts) != 2 {
			return fmt.Errorf("invalid format for --set-configmap: %q, expected key=value", set)
		}
		key := parts[0]
		value := parts[1]
		// Values are always set as strings, which is fine Viper will convert them
		// to the correct type when using type-specific getters like GetBool or GetInt.
		v.Set(key, value)
	}
...
func runDeploy(cmd *cobra.Command, args []string) error {
	...
	for _, object := range objects {
		...
		if cm, isCm := object.(*v1.ConfigMap); isCm {
			...
			err = applyConfigToConfgMap(cm, configmapOverridePath, cmd.PersistentFlags())
			if err != nil {
				return fmt.Errorf("merging config with %q: %w", configmapOverridePath, err)
			}
		}
