...
func TestApplyConfigToConfgMap(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Patch global setConfigmap for this test
			oldSetConfig := setConfigmap
			setConfigmap = tt.setConfig
			defer func() { setConfigmap = oldSetConfig }()
			...
			err := applyConfigToConfgMap(tt.cm, configPath, fs)
			if tt.wantErr {
				if err == nil {
