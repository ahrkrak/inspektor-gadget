...
You can specify a custom public key at deploy time using `--config`. Start by creating configuration file `~/.ig/deploy.yaml` with the following content:
...
Then, you can deploy Inspektor Gadget with your public key:

```bash

$ kubectl gadget deploy --configmap-override-path=$HOME/.ig/deploy.yaml
...
Inspektor Gadget successfully deployed
...
Like `kubectl-gadget`, you cannot specify information with regard to verification when calling `gadgetctl run`:

```bash
$ gadgetctl run --public-keys="$(cat your-key.pub)" trace_exec
Error: unknown flag: --public-keys
$ gadgetctl run --verify-image=false trace_exec
Error: unknown flag: --verify-image
```

Instead, all these information are set once when starting `ig` as a daemon.
This avoids them being tampered by any users.
By default all image-based gadgets are verified using Inspektor Gadget public key:

```bash
$ sudo ig daemon --public-keys="$(cat your-key.pub),$(cat inspektor-gadget.pub)"
...
...
You can also make Inspektor Gadget skip the verification on the daemon side.
Please note that we do not recommend doing so and that it should only be used for development purposes:
...
# Switch to another terminal
$ gadgetctl run ghcr.io/your-repo/gadget/trace_open:latest
WARN[0000] your_ip              | image signature verification is disabled due to using corresponding option
RUNTIME.CONTAINERN… TIMESTAMP  PID        UID        GID        MNTNS_ID   ERR        FD         FLAGS      MODE      COMM      FNAME
```
