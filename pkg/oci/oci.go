...
func getSignature(ctx context.Context, repo oras.Target, signatureTag string) ([]byte, string, error) {
...
func getPayload(ctx context.Context, repo oras.Target, payloadTag string) ([]byte, error) {
...
func pullSigningInformation(ctx context.Context, repo *remote.Repository, imageDigest string, imageStore oras.Target) error {
	...
	// copy the signature from repo:signatureTag to imageStore
...
func loadSigningInformation(ctx context.Context, imageRef reference.Named, imageStore oras.Target, authOpts *AuthOptions) ([]byte, []byte, error) {
	...
	signature, payloadTag, err := getSignature(ctx, imageStore, signatureTag)
	if errors.Is(err, errdef.ErrNotFound) {
		repo, err := newRepository(imageRef, authOpts)
		if err != nil {
			return nil, nil, fmt.Errorf("creating remote repository: %w", err)
		...
		if err := pullSigningInformation(ctx, repo, imageDigest, imageStore); err != nil {
			return nil, nil, fmt.Errorf("pulling signature: %w", err)
		}
		signature, payloadTag, err = getSignature(ctx, imageStore, signatureTag)
		if err != nil {
			return nil, nil, fmt.Errorf("getting signature after pulling: %w", err)
		}
	} else if err != nil {
		return nil, nil, fmt.Errorf("getting signature: %w", err)
	}

	payload, err := getPayload(ctx, imageStore, payloadTag)
	if err != nil {
